                    ...
                    <div class="product-img border-bottom bg-white">
                      <div class="card-image-container position-relative overflow-hidden">
                        <img src="../assets/images/products/${
                          product.main_image
                        }" class="card-img-top main-image" alt="${
            product.title
          }">
                        <img src="../assets/images/products/${
                          product.hover_image
                        }" class="card-img-top hover-image position-absolute top-0 start-0 w-100 h-100" alt="${
            product.title
          }">
                      </div>
                    </div>
                    <div class="card-body bg-body-tertiary d-flex flex-column gap-1 py-2">
                      <div class="product-vendor text-muted">${
                        product.vendor
                      }</div>
                      <div class="product-title fw-bold">${product.title}</div>
                      <ul class="list-unstyled mt-auto mb-0">
                        <li><span class="text-dark-emphasis fw-bold">$${product.price.toLocaleString(
                          "es-AR"
                        )}</span></li>
                        <li>${
                          product.installments_quantity
                        } cuotas de: <span class="text-dark-emphasis">$${Math.round(
            product.price / product.installments_quantity
          ).toLocaleString("es-AR")}</span></li>
                        <li><span class="text-dark-emphasis fw-bold">$${(
                          Math.round(product.price * 0.0085) * 100
                        ).toLocaleString(
                          "es-AR"
                        )}</span> <small class="text-muted">con Transferencia</small></li>
                      </ul>
                    </div>
                    </div>
                    <div class="m-0 p-0 z-2">
                      <div class="buy-button bg-body-tertiary top-0 z-2">
                        <div class="p-4 pt-1 z-2">
                          <a href="./producto.html?id=${
                            product.product_id
                          }" class="btn btn-warning w-100 z-5" role="button">
                            Ver producto
                          </a>
                        </div>
                      </div>
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="card card-image-container position-relative overflow-hidden">
                    <img src="../assets/images/products/${
                      product.main_image
                    }" class="card-img-top main-image" alt="${product.title}">
                    <img src="../assets/images/products/${
                      product.hover_image
                    }" class="card-img-top hover-image position-absolute top-0 start-0 w-100 h-100" alt="${
            product.title
